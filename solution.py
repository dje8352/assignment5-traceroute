    ...
    for ttl in range(1, MAX_HOPS):
        ...
        for tries in range(TRIES):
            ...
            try:
                ...
                #Fill in start
                icmpHDR = recvPacket[20:28]
                types, code, checksum, packetID, sequence = struct.unpack("bbHHh", icmpHDR)
                #Fill in end
                try: #try to fetch the hostname
                    #Fill in start
                    hopName = gethostbyaddr(hopAddr)[0]
                    #Fill in end
                except herror:   #if the host does not provide a hostname
                    #Fill in start
                    hopName = "hostname not returnable"
                    #Fill in end
                ...
                if types == 11:
                    ...
                    # You should add your responses to your lists here
                    #tracelist2.append(tracelist1)
                    tracelist2.append(["Destination host unreachable"])
                    #Fill in end
                elif types == 0:
                    bytes = struct.calcsize("d")
                    timeSent = struct.unpack("d", recvPacket[28:28 + bytes])[0]
